...
function HTMLParser(html: any, handler: any) {
  ...
  while (html) {
    ...
    if (!stack.last() || !special[stack.last()]) {
      ...
      html = html.replace(new RegExp('([\\s\\S]*?)<\/' + stack.last() + '[^>]*>'), function (all: any, text: any) {
  ...
  function parseStartTag(tag: any, tagName:any, rest:any, unary:any) {
    ...
    if (handler.start) {
      var attrs: any = [];
      rest.replace(attr, function (match: any, name: any) {
  ...
  function parseEndTag(tag: any, tagName: any) {
