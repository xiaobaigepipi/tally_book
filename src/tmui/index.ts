...
let $tm = {
	tabBar: tabBar,
	pages: pages,
	globalNavStyle: (PageJsonInit?.globalStyle.navigationStyle == 'custom'? 'custom' : 'default') as ("custom" | "default"),
...
export default {
	...
	install: (app: App, options: Tmui.tmuiConfig = {}) => {
		...
		options = util.deepObjectMerge<Tmui.tmuiConfig>($tm.config, options)
